...
export async function runOmdbBackfill(): Promise<{ updated: number }> {
  ...
  const missing = await db
    .select({ id: shows.id, imdbId: shows.imdbId, ratingImdb: shows.ratingImdb, ratingMetacritic: shows.ratingMetacritic, imdbVotes: shows.imdbVotes })
    .from(shows)
    .where(and(
  ...
  for (const row of missing) {
    ...
    try {
      ...
      const rImdb = typeof agg.imdbRating === 'number' && Number.isFinite(agg.imdbRating) ? agg.imdbRating : null;
      const rVotes = typeof agg.imdbVotes === 'number' && Number.isFinite(agg.imdbVotes) ? agg.imdbVotes : null;
      const rMc = typeof agg.metacritic === 'number' && Number.isFinite(agg.metacritic)
        ? agg.metacritic
        : (typeof agg.metascore === 'number' && Number.isFinite(agg.metascore) ? agg.metascore : null);
      await db.update(shows).set({ ratingImdb: rImdb ?? row.ratingImdb ?? null, imdbVotes: rVotes ?? row.imdbVotes ?? null, ratingMetacritic: rMc ?? row.ratingMetacritic ?? null, updatedAt: new Date() }).where(eq(shows.id, row.id));
      updated++;
    } catch {}
  }
  return { updated };
...
export async function runMetaBackfill(): Promise<{ updated: number }> {
  ...
  for (const row of missingMeta) {
    try {
      ...
      const videosFiltered = allVideos.filter((v: any) => v.site === 'YouTube' && (v.type === 'Trailer' || v.type === 'Teaser'));
      ...
      if (seasonsArr.length > 0) {
        const sorted = seasonsArr.filter((s: any) => typeof s.season_number === 'number').sort((a: any, b: any) => (b.season_number ?? 0) - (a.season_number ?? 0));
      ...
      await db.update(shows).set({
        ...
        genres: Array.isArray(tmdbShowData.genres) ? tmdbShowData.genres : ((row as any).genres ?? []),
        ...
        numberOfSeasons: (typeof tmdbShowData.number_of_seasons === 'number' ? tmdbShowData.number_of_seasons : null) ?? (row as any).numberOfSeasons ?? null,
        numberOfEpisodes: (typeof tmdbShowData.number_of_episodes === 'number' ? tmdbShowData.number_of_episodes : null) ?? (row as any).numberOfEpisodes ?? null,
