...
function DeltaRow({ s, metric, region }: { s: ShowWithUrl; metric: DeltaMetric; region?: Region }) {
  ...
  const displayTitle =
    s.title && s.titleUk && s.titleUk !== s.title
      ? `${s.title} / ${s.titleUk}`
      : s.titleUk || s.title;
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-12">
      ...
      <div className="text-xs text-gray-500 mb-2">
        Вікно: {windowDays} днів • Метрика:{' '}
        {metric === 'delta' ? 'Δ (останні знімки бази)' : 'Δ 3міс (місячні чарти)'}
      ...
      <div className="text-xs text-gray-400 bg-zinc-900/60 border border-zinc-800 rounded p-3 mb-4">
        ...
        <div className="mb-1">
          Δ — залежить від «Вікна»: різниця глядачів між найпершим і найсвіжішим знімком у межах
          вибраних {windowDays} днів. Якщо знімків бракує, показуємо різницю між двома останніми
          глобальними знімками бази.
        </div>
        <div>
          Δ 3міс — зміна за три календарні місяці, розрахована на основі місячних чартів Trakt.
          Краще відображає стабільні тренди.
        </div>
      </div>
      {gainers.length === 0 && losers.length === 0 && (
        <div className="text-xs text-gray-400 bg-zinc-900/60 border border-zinc-800 rounded p-3 mb-4">
          ...
          <div className="mb-2">
            У вибраному вікні ({windowDays} днів) не зафіксовано суттєвих «злетів» або «падінь».
            Спробуйте місячну метрику.
          </div>
          <button
