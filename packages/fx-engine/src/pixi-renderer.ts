...
import type { AchievementFxEvent, FxRenderOptions, FxRenderer } from './types';
...
import { ParticlesPlugin } from './renderer/pixi/plugins/particles-plugin';
import { SignalGlitchPlugin } from './renderer/pixi/plugins/signal-glitch-plugin';
import { createAchievementCard } from './renderer/pixi/scene/create-achievement-card';
import { createAchievementMask } from './renderer/pixi/scene/create-achievement-mask';
import { createBackdropLayers } from './renderer/pixi/scene/create-backdrop-layers';
  ...
  async playAchievement(event: AchievementFxEvent, options: FxRenderOptions): Promise<void> {
    ...
    await new Promise<void>((resolve) => {
      const tick = () => {
        ...
        const settleT = clamp(
          (elapsed - timeline.settleStartMs) / timeline.settleDurationMs,
          0,
          1,
        );
        ...
        layers.tint.alpha =
          ...
          (1 - fadeOutT);
        layers.tint.alpha +=
          signalState.inSignalCut
            ? signalState.signalStrength * (0.02 + signalState.signalRush * 0.03)
            : 0;
