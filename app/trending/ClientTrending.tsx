    ...
    async function refetch() {
      try {
        ...
        const [showsRes, airingsRes, gainersRes, losersRes] = await Promise.all([
          ...
          fetch(
            `/api/shows?limit=5&sort=${metric}&order=asc&days=${windowDays}${regionQS}${categoryQS}`
          ),
        ]);
        if (showsRes.ok) {
        ...
        if (gainersRes.ok) {
          const gj = await gainersRes.json();
          usedGainers = (gj.shows || []) as ShowWithUrl[];
        }
        if (losersRes.ok) {
          const lj = await losersRes.json();
          usedLosers = (lj.shows || []) as ShowWithUrl[];
        }
      ...
      {airings.length > 0 && (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-12">
          <AiringsList
            ...
            limit={3}
            actions={
              <a href="/airings" className="text-sm text-blue-400 hover:text-blue-300 mb-4">
                Переглянути всі →
              </a>
            }
          />
