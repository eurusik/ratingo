...
- [ ] 2. Update Ingestion to Populate Origin Data
...
- [ ] 6. Policy Repository and Service
  ...
  - [ ] 6.4 Create CatalogPolicyService
...
- [ ] 7. Evaluation Repository and Service
  - [ ] 7.1 Create MediaCatalogEvaluationRepository
  ...
  - [ ] 7.2 Create CatalogEvaluationRunRepository
  ...
  - [ ] 7.3 Create CatalogEvaluationService
...
- [ ] 8. Checkpoint - Repository tests
  - Ensure all tests pass, ask the user if questions arise

- [ ] 9. Background Jobs
  - [ ] 9.1 Create EVALUATE_CATALOG_ITEM job handler
    - Create `apps/api/src/modules/catalog-policy/application/workers/evaluate-catalog-item.worker.ts`
  ...
  - [ ] 9.2 Create RE_EVALUATE_CATALOG job handler
    - Create `apps/api/src/modules/catalog-policy/application/workers/re-evaluate-catalog.worker.ts`
...
- [ ] 11. Public Catalog Repository
  ...
  - [ ] 11.3 Create AdminCatalogRepository
...
- [ ] 12. Update Public API Endpoints
  - [ ] 12.1 Update /trending endpoint
    - Inject PublicCatalogRepository
    - Query from public_media_items view
    - _Requirements: 6.2_

  - [ ] 12.2 Update /search endpoint
    - Inject PublicCatalogRepository
    - Query from public_media_items view
    - _Requirements: 6.3_

  - [ ] 12.3 Update /discover endpoint
    - Inject PublicCatalogRepository
    - Query from public_media_items view
    - _Requirements: 6.4_

  - [ ] 12.4 Update /homepage endpoint
    - Inject PublicCatalogRepository
    - Add relevanceScore filter from active policy
    - _Requirements: 6.5_

  - [ ] 12.5 Update /details/:id endpoint
    - Return 404 for INELIGIBLE or PENDING media
    - _Requirements: 6.6_
...
- [ ] 14. Admin API Endpoints
  - [ ] 14.1 Create GET /admin/catalog/policies endpoint
    - List all policies with version history
    - _Requirements: 2.4_

  - [ ] 14.2 Create POST /admin/catalog/policies endpoint
    - Create and optionally activate new policy
    - Validate policy JSON against schema
    - _Requirements: 2.4, 10.3_

  - [ ] 14.3 Create POST /admin/catalog/policies/:id/activate endpoint
    - Activate policy, deactivate previous
    - Queue RE_EVALUATE_CATALOG job
    - _Requirements: 2.3_

  - [ ] 14.4 Create GET /admin/catalog/evaluation-runs endpoint
    - List evaluation runs with status and counters
    - _Requirements: 11.6_

  - [ ] 14.5 Create POST /admin/catalog/evaluation-runs/:id/resume endpoint
    - Resume failed evaluation run from cursor
    - _Requirements: 11.6_

- [ ] 15. Dry-Run Feature
  - [ ] 15.1 Create DryRunService
  ...
  - [ ]* 15.5 Write property test: Dry-Run Mode Item Selection
...
- [ ] 16. Observability
  ...
  - [ ] 16.3 Create CatalogAuditLogger service
  ...
  - [ ] 16.5 Integrate metrics with evaluation jobs
...
- [ ] 17. Create catalog-policy NestJS module
...
- Core property tests (4.5 Determinism, 4.10 Breakout Priority, 4.12 Relevance Range, 6.5 Policy Round-Trip, 7.4 Evaluation Round-Trip, 15.4 Dry-Run Consistency) are recommended but optional
- Unit tests (4.6-4.9) use explicit test cases (5-10 each) for readability
- Integration tests (11.2, 15.3) verify DB state
- Each property test should run minimum 100 iterations
- Use fast-check library for property-based testing in TypeScript
- STRICT eligibility mode is recommended as default
- policyVersion=0 means "no policy / pending seed"
- Default policy v1 seeded in task 6.6 provides working baseline
- Current state: No migrations, schema changes, or module code exists yet - starting from scratch
