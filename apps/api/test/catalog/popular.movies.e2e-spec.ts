...
describe('Catalog E2E - Popular Movies', () => {
  ...
  it('enriches userState for auth and null for anon', async () => {
    const anon = await ctx.get('/api/catalog/movies/popular').expect(200);
    expect(anon.body.data.data.every((m: any) => m.userState === null)).toBe(true);
    expect(anon.body.data.data.every((m: any) => m.mediaItemId && m.mediaItemId === m.id)).toBe(
      true,
    );
    expect(
      anon.body.data.data.every((m: any) => m.card && m.card.listContext === 'POPULAR_LIST'),
    ).toBe(true);

    const token = await ctx.registerAndLogin();
    await ctx.setUserState(token, 'mid-1');

    const auth = await ctx.get('/api/catalog/movies/popular', token).expect(200);
  ...
  it('applies sorting (ratingo asc) and stable tie-breaker', async () => {
    const res = await ctx.get('/api/catalog/movies/popular?sort=ratingo&order=asc').expect(200);
  ...
  it('supports sorting by releaseDate and tmdbPopularity', async () => {
    const byReleaseDesc = await ctx
      .get('/api/catalog/movies/popular?sort=releaseDate&order=desc')
      .expect(200);
    ...
    const byReleaseAsc = await ctx
      .get('/api/catalog/movies/popular?sort=releaseDate&order=asc')
      .expect(200);
    ...
    const byTmdbPopularityDesc = await ctx
      .get('/api/catalog/movies/popular?sort=tmdbPopularity&order=desc')
      .expect(200);
    ...
    const byTmdbPopularityAsc = await ctx
      .get('/api/catalog/movies/popular?sort=tmdbPopularity&order=asc')
      .expect(200);
  ...
  it('validates query params', async () => {
    ...
    for (const q of queries) {
      await ctx.get(`/api/catalog/movies/popular${q}`).expect(400);
    }
  });

  it('rejects unknown query params (forbidNonWhitelisted)', async () => {
    await ctx.get('/api/catalog/movies/popular?foo=bar').expect(400);
  });

  it('returns correct meta total/hasMore across pages', async () => {
    const page1 = await ctx.get('/api/catalog/movies/popular?limit=2&offset=0').expect(200);
    expect(page1.body.data.meta.total).toBe(4);
    expect(page1.body.data.meta.hasMore).toBe(true);

    const page2 = await ctx.get('/api/catalog/movies/popular?limit=2&offset=2').expect(200);
