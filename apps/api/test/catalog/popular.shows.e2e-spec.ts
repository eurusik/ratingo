...
describe('Catalog E2E - Popular Shows', () => {
  ...
  it('enriches userState for auth and null for anon', async () => {
    const anon = await ctx.get('/api/catalog/shows/popular').expect(200);
    ...
    expect(
      anon.body.data.data.every((s: any) => s.card && s.card.listContext === 'POPULAR_LIST'),
    ).toBe(true);

    const token = await ctx.registerAndLogin();
    await ctx.setUserState(token, 'sid-1');

    const auth = await ctx.get('/api/catalog/shows/popular', token).expect(200);
  ...
  it('returns correct meta total/hasMore across pages', async () => {
    const page1 = await ctx.get('/api/catalog/shows/popular?limit=1&offset=0').expect(200);
    expect(page1.body.data.meta.total).toBe(2);
    expect(page1.body.data.meta.hasMore).toBe(true);

    const page2 = await ctx.get('/api/catalog/shows/popular?limit=1&offset=1').expect(200);
  ...
  it('validates query params', async () => {
    ...
    for (const q of queries) {
      await ctx.get(`/api/catalog/shows/popular${q}`).expect(400);
    }
  });

  it('rejects unknown query params (forbidNonWhitelisted)', async () => {
    await ctx.get('/api/catalog/shows/popular?foo=bar').expect(400);
  });
});
