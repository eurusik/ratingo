...
describe('FacebookAuthGuard - returnTo Validation', () => {
  ...
  beforeEach(() => {
    guard = new FacebookAuthGuard(facebookConfig as any, authConfig as any);
  });

  describe('validateReturnTo (via state generation)', () => {
    // Access private method through state generation
    const getReturnToFromState = (guard: FacebookAuthGuard, returnTo?: string): string => {
  ...
  describe('canActivate - feature toggle', () => {
    it('should throw NotFoundException when Facebook OAuth is disabled', async () => {
      const disabledGuard = new FacebookAuthGuard(
        { ...facebookConfig, enabled: false } as any,
        authConfig as any,
      );
