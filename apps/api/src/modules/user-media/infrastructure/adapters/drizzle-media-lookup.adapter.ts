...
export class DrizzleMediaLookupAdapter implements IMediaLookupPort {
  ...
  async findManyByImdbIds(
    ...
    const rows = await this.db
      ...
      .where(
        and(
          inArray(schema.mediaItems.imdbId, imdbIds),
    ...
    return rows.map((r) => ({ id: r.id, imdbId: r.imdbId!, type: r.type }));
  }
  ...
  async findManyByTmdbIds(
    ...
    const rows = await this.db
      ...
      .from(schema.mediaItems)
      .where(and(inArray(schema.mediaItems.tmdbId, tmdbIds), isNull(schema.mediaItems.deletedAt)));

    return rows.map((r) => ({ id: r.id, tmdbId: r.tmdbId, type: r.type }));
  }
}
