...
export default function RunsPage() {
  ...
  const columns: DataTableColumnDef<EvaluationRunDto>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => <div className="font-medium">{row.original.policyName}</div>,
    },
    {
      id: 'status',
      ...
      cell: ({ row }) => {
        ...
        if (row.original.status === RUN_STATUS.RUNNING) {
          return (
            <span className="text-muted-foreground">
              {duration}
              {dict.admin.runs.duration.minutes} ({dict.admin.runs.duration.running})
            </span>
          );
        }
        ...
        return (
          <span className="text-sm">
            {duration}
            {dict.admin.runs.duration.minutes}
          </span>
        );
      },
    },
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">{dict.admin.runs.description}</p>
        </CardHeader>
