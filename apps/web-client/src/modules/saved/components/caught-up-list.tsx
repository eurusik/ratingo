...
import { useCaughtUp, PAGE_SIZE, MAX_LIST_LIMIT, type MeListSort } from '../hooks/use-me-lists';
...
export function CaughtUpList() {
  const { dict } = useTranslation();
  const [sort, setSort] = useState<MeListSort>('recent');
  const [limit, setLimit] = useState(PAGE_SIZE);
  const { data, isLoading, isFetching } = useCaughtUp({ sort, limit });
